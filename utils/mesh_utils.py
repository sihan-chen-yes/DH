    ...
    for i, viewpoint_cam in enumerate(viewpoint_stack):
        intrinsic = o3d.camera.PinholeCameraIntrinsic(width=viewpoint_cam.image_width,
                                                      ...
                                                      fx=viewpoint_cam.image_width / (
                                                                  2 * math.tan(viewpoint_cam.FoVx / 2.)),
                                                      fy=viewpoint_cam.image_height / (
                                                                  2 * math.tan(viewpoint_cam.FoVy / 2.)))
...
class GaussianExtractor(object):
    def __init__(self, gaussians, scene, render, pipe, bg_color=None):
        """
    ...
    def reconstruction(self, viewpoint_stack, iteration):
        ...
        point_cloud_path = os.path.join(self.scene.save_dir, "point_cloud/frame_{}".format(frame_id), "point_cloud.ply")
    ...
    #         sampled_normal = torch.nn.functional.grid_sample(normalmap.cuda()[None], pix_coords[None, None],
    #                                                          mode='bilinear', padding_mode='border',

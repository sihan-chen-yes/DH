...
def img_name_to_tensor(img_path):
    img = imageio.imread(img_path)
    img = (np.array(img) / 255.).astype(np.float32)
    img = np.transpose(img, (2, 0, 1))
...
files_dir = './exp/393_2dgs_full/test-view'


estim_dir = os.path.join(files_dir, "renders")
gt_dir = os.path.join(files_dir, "gt")

estim = read_images_in_dir(estim_dir)
gt = read_images_in_dir(gt_dir)

print('total images:', len(estim), len(gt))
assert len(estim) == len(gt)
...
for i in tqdm(range(0, len(estim), batch_size)):
    estim_batch = estim[i:i+batch_size]
    gt_batch = gt[i:i+batch_size]

    estim_batch = torch.stack([img_name_to_tensor(os.path.join(estim_dir, img_name)) for img_name in estim_batch])
    gt_batch = torch.stack([img_name_to_tensor(os.path.join(gt_dir, img_name)) for img_name in gt_batch])
...
for key in errors_all.keys():
    errors[key] = errors_all[key] / len(estim)
